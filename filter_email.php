...
$sql = "SELECT b.book_id, b.title, b.year, GROUP_CONCAT(DISTINCT a.name SEPARATOR ', ') 
    ...
    where ".$condition." (a.author_id like '%$authId%' || a.author_id is null) and (c.category_id like '%$catId%' || c.category_id is null) and(b.title like '%$searched%' || b.book_id like '%$searched%' ||  c.category_name like '%$searched%' ||    ac.account_no like '%$searched%' || a.name like '%$searched%')
    GROUP BY b.book_id";
$result = mysqli_query($connect, $sql);

while ($row = mysqli_fetch_assoc($result)) {
?><tr td class="clickable-row" data-bs-toggle="modal" data-bs-target="#bookInfoModal" data-v-book-id="<?php echo $row['book_id'] ?>" data-v-book-title="<?php echo $row['title'] ?>" data-v-book-year="<?php echo $row['year'] ?>" data-v-book-authors="<?php echo $row['authors'] ?>" data-v-book-categories="<?php echo $row['categories'] ?>" data-v-book-accounts="<?php echo $row['accountss'] ?>">
        ...
        <td style="width: 5%"><?php echo $row['no_copies'] ?></td>
        <td style="width: 5%"><?php echo $row['no_cd_copy'] ?></td>
    </tr>
<?php }
?>